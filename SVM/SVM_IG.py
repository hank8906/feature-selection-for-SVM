...
# 计算每个特征的信息增益
information_gains = mutual_info_classif(X_train_scaled, y_train)

# 打印每个特征的信息增益
# print("Information gains for each feature:")
# for i, gain in enumerate(information_gains):
#     print(f"Feature {i}: {gain}")

# 选择信息增益大于等于阈值的特征
threshold = 0.05  # 设置信息增益的阈值
selected_features_indices = np.where(information_gains >= threshold)[0]
selected_features = df.columns[:-1][selected_features_indices]

print("Infromation gain :")
# 打印选择的特征
